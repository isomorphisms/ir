...
<<eval=FALSE>>=
    fun <- function(r) statistic(data, i[r, ], ...)
    RR <- sum(R)
    res <- if (ncpus > 1L && (have_mc || have_snow)) {
        if (have_mc) {
            parallel::mclapply(seq_len(RR), fun, mc.cores = ncpus)
        } else if (have_snow) {
            ...
            if (is.null(cl)) {
                ...
                res <- parallel::parLapply(cl, seq_len(RR), fun)
                parallel::stopCluster(cl)
                res
            } else parallel::parLapply(cl, seq_len(RR), fun)
        }
    } else lapply(seq_len(RR), fun)
@
...
hence the environment of \code{fun} which will therefore be sent to the
workers along with \code{fun}.
